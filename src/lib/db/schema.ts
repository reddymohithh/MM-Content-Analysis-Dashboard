...
/**
 * Editorial content-quality score from an LLM pass against the full global
 * newsletter content-analysis checklist (Section 23's complete required
 * output: category scores, audience fit, reader outcome, story-by-story
 * analysis, batch-specific feedback, cross-batch feedback, improvement
 * plans, and more) — deliberately independent of the engagement-metric-
 * based score in quality-score.ts, since open rate/CTR/polls/unsub are
 * explicitly *not* valid content-quality signals per that checklist. One
 * row per edition, overwritten on re-score.
 */
export const contentQualityScores = pgTable("content_quality_scores", {
  ...
  /** Everything else Section 23 asks for besides the 12 category scores:
   * verdict, audienceFit, readerOutcome, storyByStory, whatWorked,
   * whatDidntWork, biggestMissedOpportunity, batch1, batch2, crossBatch,
   * recommendedImprovements, nextEditionPlan, contentOpportunities,
   * strengthsToPreserve, criticalFailures, finalSummary — see
   * ContentQualityResult in src/lib/scoring/content-quality.ts for the full
   * shape. `classification` isn't stored here since it's a pure function
   * of `total` (classifyContentQuality), recomputed on read rather than
   * persisted where it could drift from the number next to it. */
