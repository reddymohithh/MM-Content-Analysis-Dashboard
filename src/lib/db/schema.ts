...
export const contentQualityScores = pgTable("content_quality_scores", {
  ...
  /** { batch1: AudienceFeedback; batch2: AudienceFeedback } — Section 17's
   * separate practitioner/leadership feedback, generated by the same LLM
   * call. Powers the Content quality card's narrative and the "Tips and
   * suggestions" card, both switched by the page's Batch 1/Batch 2 toggle. */
  batchFeedback: jsonb("batch_feedback")
    .$type<{
      batch1: { narrative: string; tips: string[] };
      batch2: { narrative: string; tips: string[] };
    }>()
    .notNull(),
