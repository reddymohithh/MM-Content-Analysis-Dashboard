    ...
    <div className="mx-auto max-w-[1120px]">
      ...
      <div className="mb-4 grid grid-cols-2 gap-3.5">
        ...
        <div className="flex flex-col gap-3.5">
          <div className="rounded-xl border border-border bg-card p-4" style={{ flex: 3 }}>
            <PollChart poll={edition.poll} />
          </div>
          <div className="rounded-xl border border-border bg-card p-4" style={{ flex: 1 }}>
      ...
      <div className="grid grid-cols-2 gap-3.5">
        <div className="overflow-hidden rounded-[10px] border border-border bg-card">
          ...
          {editorialLinks.length === 0 ? (
            <EmptyState>No editorial link clicks recorded.</EmptyState>
          ) : (
        ...
        <div className="overflow-hidden rounded-[10px] border border-border bg-card">
          ...
          {promoted.length === 0 ? (
            <EmptyState>No promoted placements this edition.</EmptyState>
          ) : (
            promoted.map((p) => (
              <div key={p.id} className="flex items-center justify-between border-t border-hairline px-3.5 py-2 text-[12.5px] first:border-t-0">
                <span className="truncate pr-3">
                  <span className="font-semibold">{p.sponsor}</span> — {p.description}
                </span>
                <span className="flex-shrink-0 font-mono text-text-muted">{p.clicks}</span>
              </div>
            ))
          )}
        </div>
