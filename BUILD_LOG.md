...
## Round 53: sample content-quality preview while the real key is pending

Explained how the "Why this edition scored X% engagement" section's
Blended/Batch 1/Batch 2 toggle actually works (`quality-score.ts`) --
the underlying score and its four weighted components (satisfaction,
engagement, retention, voice) are identical regardless of lens; only
the narrative wording and each component's "why" sentence change per
audience (`whyFor()`), an editorial reframing, not a different
computation. Purely rule-based, unrelated to the LLM checklist.

User then asked to see a sample of what the LLM ranking will look like
in the dashboard. Two readings were possible -- a cross-edition
leaderboard, or a single edition's scored panel -- so asked which
before building either. User picked the single-edition panel.

Built `src/lib/scoring/sample-content-quality.ts`: static example data
covering all 12 real categories, run through the actual
`computeContentQualityTotal()` weighting function so the total and
effective weights are internally consistent with how a real score
would compute, not hand-typed numbers. Deliberately written
edition-agnostic ("the piece," "this story") rather than referencing
any specific real edition's content, since the same static sample
renders on every unanalyzed edition's page regardless of what that
edition actually covers -- referencing a real headline would have read
as wrong on every other edition's page.

Rendered on the edition detail page only inside the existing "not
analyzed yet" branch of both the Content quality (editorial) and Tips
and suggestions cards, never replacing the honest empty-state message,
always below it. Wrapped in a dashed border distinct from every other
card's solid border, with a persistent "SAMPLE PREVIEW, NOT REAL DATA"
badge, and the sample's own `model` field reads "Sample only, not yet
scored" in the exact slot where a real model name would otherwise
show -- three separate, redundant signals that this isn't real data,
given the standing rule against ever presenting fabricated numbers as
genuine.

**Verified in the browser**: opened LinkedIn's New AI Citation
Playbook (a different, unrelated edition from every prior verification
in this session) and confirmed the sample text reads sensibly without
referencing anything Crocs-mascot-specific left over from writing it,
confirmed the dashed border and badge render on both cards, and
